...
The `my-main` branch includes a host-based production workflow where `sub2api`, Nginx, PostgreSQL, and Redis are managed directly by systemd.

Run the complete manual release workflow from the repository root:

```bash
DEPLOY_TARGET=us1 ./deploy/ops/release-us1-from-main.sh
```

The workflow synchronizes `upstream/main` to the Fork, merges it into `my-main`, pushes `my-main`, builds both release packages, deploys them, and runs health checks. A merge conflict stops the workflow before build or deployment.

See [`ops/README.md`](./ops/README.md) for server paths, individual commands, backup operations, and failure handling.
